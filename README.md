...
- Kuan, Chin-Wei (官京緯): Producer => Part B
...
1. The pool submit() function is partially implemented and currently
...
1. Theworker()functionisexecutedbyeachthreadinthepool,whereeach
...
P-37
1. A mutex lock is necessary to avoid race conditions when accessing or
modifying the queue. (Section 7.3.1 provides details on Pthreads mutex
locks.)
1. Thepool shutdown()function will cancel each worker thread and then
